    ...
    @section('content')
        ...
        <div class="home">
            <div class="home__carousel owl-carousel" id="flixtv-hero">
                ...
                <div class="home__card">
                    <a href="{{route('single_serial',$serial->original_title)}}">
                        <img src="{{$serial->img}}" alt="">
                    </a>
                    <div>
                        <h2>{{$serial->original_title}}</h2>
                        @php
                            $genres = $genre_serial->where('serial_id' , $serial->serial_id)->all();
                        @endphp
                        <ul class="card__list">
        ...
        <div class="catalog">
            <div class="container">
                        ...
                        <div class="catalog__nav">
                            <div class="catalog__select-wrap">
                                <div class="m-3">
                                    ...
                                    <a href="/{{ $category_serial_title->slug}}" class="header__nav-link">{{$category_serial_title->title}}</a>
                                </div>



                                @foreach($categories_serial as $genre)
                                <div class="m-3">
                                    <form action="{{route('category-child-serial' , $genre->title)}}" method="post">
                                        @csrf
                                        <input type="hidden" name="category" value="{{$genre->title}}">
                                       <button class="btn-sm" style="color: wheat; font-size: 14px">{{$genre->title}}</button>
                                    </form>
                            ...
                            <div class="slider-radio">
                                <input type="radio" name="grade" id="newest"><label for="newest">جدید</label>
                            </div>
                        </div>
                ...
                <div class="row">
                    <div class="col-12">
                        <a href="{{route('category-child-serial' , $genre->title)}}" class="catalog__more">نمایش بیشتر</a>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="catalog">
            <div class="container">
                <div class="row">
                    <div class="col-12">
                        <div class="catalog__nav">
                            <div class="catalog__select-wrap">
                                <div class="m-3">
                                    <a href="/{{ $category_film_title->slug}}" class="header__nav-link">{{$category_film_title->title}}</a>
                                </div>



                                @foreach($categories_video as $genre)
                                    <div class="m-3">
                                        <form action="{{route('category-child-video' , $genre->title)}}" method="post">
                                            @csrf
                                            <input type="hidden" name="category" value="{{$genre->title}}">
                                            <button class="btn-sm" style="color: wheat; font-size: 14px">{{$genre->title}}</button>
                                        </form>
                            ...
                            <div class="slider-radio">
                                <input type="radio" name="grade" id="newest"><label for="newest">ویژه</label>
                            </div>
                        </div>
                        ...
                        <div class="row row--grid">
                            @foreach($videos as $video)
                                <div class="col-6 col-sm-4 col-lg-3 col-xl-2">
                                    <div class="card">
                                        <a href="{{route('single_film',$video->original_title)}}" class="card__cover">
                                        ...
                                        <span class="card__rating"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M22,9.67A1,1,0,0,0,21.14,9l-5.69-.83L12.9,3a1,1,0,0,0-1.8,0L8.55,8.16,2.86,9a1,1,0,0,0-.81.68,1,1,0,0,0,.25,1l4.13,4-1,5.68A1,1,0,0,0,6.9,21.44L12,18.77l5.1,2.67a.93.93,0,0,0,.46.12,1,1,0,0,0,.59-.19,1,1,0,0,0,.4-1l-1-5.68,4.13-4A1,1,0,0,0,22,9.67Zm-6.15,4a1,1,0,0,0-.29.88l.72,4.2-3.76-2a1.06,1.06,0,0,0-.94,0l-3.76,2,.72-4.2a1,1,0,0,0-.29-.88l-3-3,4.21-.61a1,1,0,0,0,.76-.55L12,5.7l1.88,3.82a1,1,0,0,0,.76.55l4.21.61Z"/></svg> 8.3</span>
                                        <h3 class="card__title"><a href="{{route('single_film',$video->original_title)}}">{{$video->original_title}}</a></h3>
                                        @php
                                            $genres = $genre_video->where('video_id' , $video->video_id)->all();
                                        @endphp
                                        <ul class="card__list">
                                            @foreach($genres as $genre)
                ...
                <div class="row">
                    <div class="col-12">
                        <a href="/{{ $category_film_title->slug}}" class="catalog__more">نمایش بیشتر</a>
                    </div>
                </div>
            </div>
        </div>
